...
*) 	messageBox "Ошибка: введён некорректный ключ." "1"
	...
	-o: Оптимизация профиля			-p: Без проверки обновлений
	-r: Перезапуск службы			-R: Удаление пакета
	-s: Остановка службы			-S: Запуск службы
	-u: Обновление NK			-U: Обновление пакета
	-v: Текущая версия NK			-W: Установка Web-интерфейса
	-z: Предварительная настройка маршрутизатора с KeeneticaOS 2.x"
	exit
	;;
